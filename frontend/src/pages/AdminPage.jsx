...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState('document'); // document | providers | slots | status
  ...
  const fetchUploadedDocs = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/upload/documents?limit=20`);
      ...
      <aside className={`admin-sidebar ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
        <div className="admin-sidebar-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1rem', color: 'var(--text-main)', textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>HĐ tuyển sinh</h2>
            </div>
          </div>
        </div>
